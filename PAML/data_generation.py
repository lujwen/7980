...
# creates two maps: user id --> list of items id the have interacted with, item id --> set of user IDs that have interacted with it
...
# get the co-click neighbors for a given set of items,co-click neighbors are users interacted with the same items as target user.
# computes a similarity score based on the interactions, return max_num most similar nighbors 
...
# get all other users interacted with a specific item
...
# get all items sharing features with a specific item iid(identifying similar items based on shared features),
# return prob distribution over the similar items
# k: v --> iid of the item similar to the input item: similar score (example:{1: 0.15, 10, 0.10})

#similarity score: 1:probability of each feature belonging to the input item(1/ number of features the input itme has)
#                  2:probability of each similar item sharing the feature with the input item(1/num of items shareing that feature)
...
# #modify
# calculate the similarity between two users if they are in same groups, return similarity score, if not, return 0 by default
def get_group_similarity(uid1, uid2, user_groups_list):
    ...
    shared_groups = len(uid1_groups & uid2_groups)
    total_groups = len(uid1_groups | uid2_groups)
    if total_groups == 0:
        return 0
    return shared_groups / total_groups

    
    # uid_group_similarity = collections.defaultdict(float)
    # uid_uids = set()
    # co_clicked_items = uid_iids_map[uid]
    # for iid in co_clicked_items:
    #     uids = iid_uids_map[iid]
    #     if uid in uids:
    #         uids = iid_uids_map[iid]- set(uid)
    #     uid_uids.update(uids)
    # for id in uid_uids:

    # similarity = shared groups / total groups
    # return uid_group_prob



#get implicit neighbors for a single user(uid), return a ranked list of implicit users and their scores, 
def get_implicit_neighbor_for_single(uid, rate_uid_iid_dict, rate_iid_uid_dict, feat_iid_fid_dict, feat_fid_iids_dict, user_groups_list):
    """
    :param uid:
...
# end modify
#
def get_heterogeneous_graph(dataset_path, item_feat_list, rate_list): # two palindrom paths
    """
    :param dataset_path:
...
#creates user2id and item2id mappings, which convert user and item IDs to index values
#creates userFeat2id and itemFeat2id, which map user and item feature values to indices
#saves these mappings to the output_dir as pickle files
...
#convert user and item feature values into indices, save indexed features to the output_dir
...
#creates a mapping from each user to their neighbors, save the result as a pickle file
...
#generate a tensor representation of the features abd labels for a batch of users
...
#generate training samples for the dataset
#modify
   ...
   #modify    
            ...
            for neighbor in neighbor_list:
                if neighbor in meta_training_feat_dict:
                    ...
                    # print(f"Warning: Neighbor {neighbor} found in meta_training_feat_dict.")
                # else:
                #     # print(f"Warning: Neighbor {neighbor} not found in meta_training_feat_dict.")
                #     print("...")
            ...
            for coclick in coclick_list:
                if coclick in meta_training_feat_dict:
                ...
                #     print(f"Warning: Coclick neighbor {coclick} found in meta_training_feat_dict.")
                # else:
                #     # print(f"Warning: Coclick neighbor {coclick} not found in meta_training_feat_dict.")
                #     print(">>>")
...
# debugging
    ...
    #modify
                  'user_groups_list':dataset.user_groups_list}
    
    # user_power_score = dataset.get_user_power_score(base_groups = 2, base_score = 1.5, k = 1)
    get_training_samples(dataset_path, user2id, item2id, user_feature, item_feature, output_dir, **other_data)
    
    #end modify

# acerage number of neighbors and co-click neighbors per user, along with other summary statistics
